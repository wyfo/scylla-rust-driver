...
impl Session {
    ...
    pub async fn query(
        ...
        let result = self.unlogged_query(query, values).await;
        match &result {
    ...
    pub async fn execute(
        ...
        let now = Instant::now();
        self.metrics.inc_total_nonpaged_queries();
        let result = self.unlogged_execute(prepared, values).await;
        match &result {
            Ok(_) => self.log_latency(now.elapsed().as_millis() as u64),
            Err(_) => self.metrics.inc_failed_nonpaged_queries(),
        };
